...
uint32_t imm_to_word_unsigned(const char *immStr) {
    ...
    // Return full 32-bit value (truncated to what fits)
...
AssembledOperation assemble_jumps(const Instruction *instruction,
  ...
  // JAL rd, imm - 3 operands, imm is 20-bit
  int count = sscanf(asmLine, "%*s %[^,], %s", rdStr, immStr);
  if (count != 2) {
    return (AssembledOperation){.hasValue = false};
  }
  ...
  uint32_t rd = register_to_byte(rdStr);
  uint32_t imm = imm_to_word_unsigned(immStr) & 0xFFFFF; // 20-bit for JAL

  uint32_t insOp = 0;
  insOp |= (instruction->funct3 & 0b00000111); // bits 0-2 (funct3=0x01)
  insOp |= (instruction->opcode & 0b00011111) << 3; // bits 3-7 (opcode=0x06)
  insOp |= (rd & 0b00001111) << 8; // bits 8-11 (rd)
  // imm[19:0] fits in bits 12-31
  insOp |= (imm & 0xFFF) << 12; // bits 12-23 (lower 12 bits of imm)
  insOp |= ((imm >> 12) & 0xFF) << 24; // bits 24-31 (upper 8 bits of imm)
...
AssembledOperation assemble_jumps_register(const Instruction *instruction,
  ...
  uint32_t imm = imm_to_word_unsigned(immStr) & 0xFFFF; // 16-bit for JALR

  uint32_t insOp = 0;
  insOp |= (instruction->funct3 & 0b00000111); // bits 0-2 (funct3=0x02)
  insOp |= (instruction->opcode & 0b00011111) << 3; // bits 3-7 (opcode=0x06)
  insOp |= (rd & 0b00001111) << 8; // bits 8-11 (rd)
